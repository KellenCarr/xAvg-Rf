...
def whoami(request: Request) -> dict:
    ...
    return {"role": ident.role, "actor": ident.actor}
...
def get_entry(namespace: str, name: str) -> dict:
    try:
...
def get_payload(namespace: str, name: str) -> StreamingResponse:
    try:
...
async def submit_entry(
    ...
    # Force pending on submission
    m.status = "pending"
    m.reviewer = None
    storage.write_manifest(m)
    if payload_file is not None:
        # Accept a single file as payload/SKILL.md if provided
